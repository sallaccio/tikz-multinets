...
In general, a multicell with coarity $1$ and arity $k$ will be replaced by a simple cell with arity $k$.
...
Nevertheless, the \lq\lq normal\rq\rq\ way of doing is to draw only ports that are not connected to any wire.
...
\DescribeMacro{pax} A survivor from \inetPackage: that port is in the middle even if the cell has even arity.
\begin{exampleBox}[sidebyside, righthand width=3cm]
\begin{verbatim}
\inetmulticell{a}{5}{4};
\inetport(a.pax)
